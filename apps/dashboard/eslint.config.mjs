import { defineConfig, globalIgnores } from "eslint/config";
import nextVitals from "eslint-config-next/core-web-vitals";
import nextTs from "eslint-config-next/typescript";

const eslintConfig = defineConfig([
  ...nextVitals,
  ...nextTs,
  {
    files: ["src/app/guardian/page.tsx"],
    rules: {
      // Guardian hydrates an owner token from the browser's
      // external sessionStorage after mounting. The state update
      // is intentionally scoped to this one client-only page.
      "react-hooks/set-state-in-effect": "off",
    },
  },
  {
    files: ["src/app/company/flow/page.tsx"],
    rules: {
      // The live topology subscribes to external platform truth on mount
      // and every 15 seconds. Its refresh state is intentionally updated
      // as part of that external polling lifecycle.
      "react-hooks/set-state-in-effect": "off",
    },
  },
  // Override default ignores of eslint-config-next.
  globalIgnores([
    // Default ignores of eslint-config-next:
    ".next/**",
    "out/**",
    "build/**",
    "next-env.d.ts",
  ]),
]);

export default eslintConfig;
