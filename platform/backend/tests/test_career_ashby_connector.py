...
_FIXTURE = Path(__file__).parent / "fixtures" / "career" / "ashby_job_board.json"
...
def _parse_input(
    ...
    *,
    ...
    return CareerConnectorParseInput(
        ...
        content_evidence_id=("internet-content-89abcdef0123456701234567"),
        source_url=(source_url or _connector().jobs_url),
...
def test_descriptor_contract() -> None:
    ...
    assert ASHBY_CONNECTOR_ID == "career-connector-ashby-job-board-v1"

    assert descriptor.connector_id == ASHBY_CONNECTOR_ID

    assert descriptor.connector_kind == "ashby"

    assert descriptor.response_media_types == ("application/json",)
    ...
    assert connector_exports.ASHBY_CONNECTOR_ID == ASHBY_CONNECTOR_ID

    assert connector_exports.AshbyJobBoardConnector is AshbyJobBoardConnector

    assert connector_exports.AshbyConnectorParseError is AshbyConnectorParseError
...
def test_valid_listed_jobs_parse() -> None:
    result = _connector().parse_candidates(_parse_input())
...
def test_unlisted_job_is_excluded() -> None:
    ...
    assert all(
        candidate.title_hint != "Internal Direct Link Role"
...
def test_missing_isListed_is_excluded() -> None:
    ...
    payload["jobs"].append(
        {
            ...
            "jobUrl": (
                "https://jobs.ashbyhq.com/acme/44444444-4444-4444-8444-444444444444"
            ),
        }
    )
    ...
    result = _connector().parse_candidates(_parse_input(text))

    assert result.candidate_count == 2

    assert all(
        candidate.title_hint != "Missing Listing State"
        for candidate in result.candidates
    )
...
def test_job_url_is_documented_source_identity() -> None:
    ...
    assert candidate.source_job_id == candidate.detail_url

    assert candidate.source_job_id == (
        "https://jobs.ashbyhq.com/acme/11111111-1111-4111-8111-111111111111"
    )

    assert candidate.source_identity_key.startswith("career-source-key-")
...
def test_invalid_published_at_fails_closed() -> None:
    ...
    payload["jobs"][0]["publishedAt"] = "2026-08-20"
    ...
    with pytest.raises(
        ...
        _connector().parse_candidates(_parse_input(text))


def test_evidence_binding_is_preserved() -> None:
    parse_input = _parse_input()

    result = _connector().parse_candidates(parse_input)

    assert result.research_evidence_id == parse_input.research_evidence_id

    assert result.content_evidence_id == parse_input.content_evidence_id

    assert result.normalized_text_sha256 == parse_input.normalized_text_sha256

    for candidate in result.candidates:
        assert (
            candidate.discovery_research_evidence_id == parse_input.research_evidence_id
        )

        assert (
            candidate.discovery_content_evidence_id == parse_input.content_evidence_id
        )
