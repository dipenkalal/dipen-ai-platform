...
def _parse_input() -> CareerConnectorParseInput:
    return CareerConnectorParseInput(
        ...
        source_url=("https://api.example.test/jobs"),
...
def _candidate(
    *,
    ...
    return CareerDiscoveryCandidate.build(
        connector_id=("career-connector-greenhouse"),
        connector_kind="greenhouse",
        employer_name="Acme",
        source_job_id="123",
        title_hint=title,
        location_hint="Ontario, Canada",
        detail_url=("https://jobs.example.test/acme/123"),
        apply_url_hint=("https://jobs.example.test/acme/123/apply"),
        posted_at_hint=(NOW - timedelta(hours=5)),
        discovery_research_evidence_id=(RESEARCH_EVIDENCE_ID),
        discovery_content_evidence_id=(CONTENT_EVIDENCE_ID),
        discovery_normalized_text_sha256=(NORMALIZED_SHA),
...
def test_descriptor_rejects_unsealed_vendor_json() -> None:
    with pytest.raises(
        ...
        CareerConnectorDescriptor(
            ...
            display_name="Greenhouse",
            priority=1,
            response_media_types=("application/vnd.example+json",),
        )
...
def test_parse_input_requires_matching_content_hash() -> None:
    ...
    assert parsed.normalized_text_sha256 == NORMALIZED_SHA
...
def test_parse_input_rejects_hash_mismatch() -> None:
    with pytest.raises(
        ValidationError,
        match="parser_text_sha256 does not match parser_text",
    ):
        CareerConnectorParseInput(
            source_body_sha256=("1" * 64),
            research_evidence_id=(RESEARCH_EVIDENCE_ID),
...
def test_parse_input_requires_https_without_credentials() -> None:
    with pytest.raises(
        ...
        CareerConnectorParseInput(
            ...
            research_evidence_id=(RESEARCH_EVIDENCE_ID),
            content_evidence_id=(CONTENT_EVIDENCE_ID),
            source_url=("http://api.example.test/jobs"),
            media_type="application/json",
            normalized_text=NORMALIZED_TEXT,
            normalized_text_sha256=NORMALIZED_SHA,
            observed_at=NOW,
    ...
    with pytest.raises(
        ...
        CareerConnectorParseInput(
            ...
            research_evidence_id=(RESEARCH_EVIDENCE_ID),
            content_evidence_id=(CONTENT_EVIDENCE_ID),
            source_url=("https://user:secret@api.example.test/jobs"),
            media_type="application/json",
            normalized_text=NORMALIZED_TEXT,
            normalized_text_sha256=NORMALIZED_SHA,
            observed_at=NOW,
...
def test_candidate_rejects_non_https_detail_url() -> None:
    with pytest.raises(
        ...
        CareerDiscoveryCandidate.build(
            ...
            detail_url=("http://jobs.example.test/acme/123"),
            discovery_research_evidence_id=(RESEARCH_EVIDENCE_ID),
            discovery_content_evidence_id=(CONTENT_EVIDENCE_ID),
            discovery_normalized_text_sha256=(NORMALIZED_SHA),
            observed_at=NOW,
...
def test_connector_result_rejects_cross_evidence_candidate() -> None:
    ...
    wrong_candidate = CareerDiscoveryCandidate.build(
        connector_id=("career-connector-greenhouse"),
        connector_kind="greenhouse",
        employer_name="Acme",
        source_job_id="123",
        title_hint="Junior Cloud Engineer",
        detail_url=("https://jobs.example.test/acme/123"),
        discovery_research_evidence_id=("research-retrieval-333333333333333333333333"),
        discovery_content_evidence_id=(CONTENT_EVIDENCE_ID),
        discovery_normalized_text_sha256=(NORMALIZED_SHA),
        observed_at=NOW,
    )
