...
FIXTURE = Path(__file__).parent / "fixtures" / "career" / "greenhouse_jobs.json"

RESEARCH_EVIDENCE_ID = "research-retrieval-111111111111111111111111"

CONTENT_EVIDENCE_ID = "internet-content-222222222222222222222222"
...
def test_descriptor_has_no_network_or_submission_authority() -> None:
    ...
    assert descriptor.connector_id == GREENHOUSE_CONNECTOR_ID
...
def test_fixture_parses_jobs_and_skips_prospect_post() -> None:
    ...
    assert [item.source_job_id for item in result.candidates] == [
        "900001",
        "900002",
    ]
...
def test_greenhouse_updated_at_is_not_posted_at() -> None:
    ...
    assert first.source_updated_at_hint is not None

    assert first.source_updated_at_hint.isoformat() == "2026-08-20T15:00:00-04:00"
...
def test_candidate_location_and_detail_url_are_preserved() -> None:
    ...
    assert first.detail_url == ("https://boards.greenhouse.io/acme/jobs/900001")
...
def test_repeated_parse_is_deterministic() -> None:
    ...
    first = connector.parse_candidates(parse_input)

    second = connector.parse_candidates(parse_input)
...
def test_wrong_greenhouse_source_url_is_rejected() -> None:
    with pytest.raises(
        ...
        _connector().parse_candidates(
            _parse_input(
                source_url=("https://boards-api.greenhouse.io/v1/boards/other/jobs")
            )
        )
...
def test_non_json_media_type_is_rejected() -> None:
    with pytest.raises(
        ...
        _connector().parse_candidates(_parse_input(media_type="text/plain"))
...
def test_meta_total_mismatch_fails_closed() -> None:
    ...
    with pytest.raises(
        ...
        _connector().parse_candidates(_parse_input(payload))


def test_board_job_ceiling_fails_closed() -> None:
    payload = _fixture_payload()

    payload["meta"]["total"] = GREENHOUSE_MAX_JOBS + 1
...
def test_insecure_greenhouse_absolute_url_fails_closed() -> None:
    ...
    payload["jobs"][0]["absolute_url"] = "http://boards.greenhouse.io/acme/jobs/900001"
