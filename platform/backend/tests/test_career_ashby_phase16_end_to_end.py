...
@pytest.mark.asyncio
async def test_ashby_fixture_flows_phase16_to_career_candidates() -> None:
    (
        orchestrator,
        retriever,
        repository,
    ...
    assert result.connector_id == ASHBY_CONNECTOR_ID
    assert result.source_url == connector.jobs_url
    assert result.candidate_count == 2
    assert [c.title_hint for c in result.candidates] == ['Junior Cloud Engineer', 'Cloud Support Associate']
    assert [c.source_job_id for c in result.candidates] == ['https://jobs.ashbyhq.com/acme/11111111-1111-4111-8111-111111111111', 'https://jobs.ashbyhq.com/acme/33333333-3333-4333-8333-333333333333']
    assert all(c.source_job_id == c.detail_url for c in result.candidates)
    for excluded_url in ['https://jobs.ashbyhq.com/acme/22222222-2222-4222-8222-222222222222']:
        assert all(c.source_job_id != excluded_url for c in result.candidates)

@pytest.mark.asyncio
async def test_ashby_e2e_preserves_phase16_evidence_binding() -> None:
    (
        orchestrator,
        _retriever,
        repository,
...
@pytest.mark.asyncio
async def test_ashby_e2e_preserves_published_at_and_truth_fail_closed() -> None:
    (
        orchestrator,
        _retriever,
        _repository,
    ...
    assert result.candidate_count == 2
    timestamp_candidate = next(c for c in result.candidates if c.source_job_id == 'https://jobs.ashbyhq.com/acme/11111111-1111-4111-8111-111111111111')
    no_timestamp_candidate = next(c for c in result.candidates if c.source_job_id == 'https://jobs.ashbyhq.com/acme/33333333-3333-4333-8333-333333333333')
    expected_published_at = datetime.fromisoformat('2026-08-20T14:30:00Z'.replace("Z", "+00:00"))
    assert timestamp_candidate.posted_at_hint == expected_published_at
    assert no_timestamp_candidate.posted_at_hint is None
    for candidate in result.candidates:
...
@pytest.mark.asyncio
async def test_ashby_e2e_rejects_truncated_or_invalid_structured_evidence() -> None:
    ...
    # invalid JSON can reach the Ashby parser.
    assert len(repository.records) == 1
