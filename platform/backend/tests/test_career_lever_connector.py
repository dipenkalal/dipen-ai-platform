...
RESEARCH_ID = "research-retrieval-111111111111111111111111"

CONTENT_ID = "internet-content-222222222222222222222222"

FIXTURE = Path(__file__).parent / "fixtures" / "career" / "lever_postings.json"
...
def _parse_input(
    *,
    ...
    return CareerConnectorParseInput(
        ...
        research_evidence_id=(RESEARCH_ID),
...
def test_descriptor_contract() -> None:
    ...
    assert LEVER_POSTINGS_API_HOST == "api.lever.co"

    assert LEVER_CONNECTOR_ID == "career-connector-lever-postings-api-v1"

    assert descriptor.connector_id == LEVER_CONNECTOR_ID

    assert descriptor.connector_kind == "lever"

    assert descriptor.display_name == "Lever Postings API"
...
def test_valid_fixture_parses_two_candidates() -> None:
    ...
    assert {candidate.source_job_id for candidate in result.candidates} == {
...
def test_core_lever_field_mapping() -> None:
    ...
    by_id = {candidate.source_job_id: candidate for candidate in result.candidates}

    first = by_id[("1a111111-1111-4111-8111-111111111111")]

    assert first.title_hint == "Junior Cloud Engineer"

    assert first.location_hint == "Toronto, Ontario, Canada"

    assert first.detail_url == (
        "https://jobs.lever.co/acme/1a111111-1111-4111-8111-111111111111"
    )

    assert first.apply_url_hint == first.detail_url + "/apply"
...
def test_apply_url_is_navigation_hint_only() -> None:
    ...
    assert result.candidates[0].apply_url_hint is not None

    assert result.candidates[0].application_authority_granted is False
...
def test_cross_site_source_provenance_rejected() -> None:
    with pytest.raises(
        ...
        _connector().parse_candidates(
            _parse_input(
                source_url=("https://api.lever.co/v0/postings/other?mode=json"),
            )
        )
    ...
    payload.append(
        {
            ...
            "hostedUrl": ("https://jobs.lever.co/acme/missing-id"),
        }
    )
...
def test_connector_has_no_network_database_browser_or_submission_authority() -> None:
    path = Path("career/connectors/lever.py")
    ...
    connector_class = next(
        ...
        if (
            ...
            and node.name == "LeverJobSiteConnector"
        )
    )
    ...
    assert (methods & forbidden_methods) == set()
