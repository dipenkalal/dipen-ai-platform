...
class ChatHistoryService:
    ...
    def create_conversation(
        ...
        return self.repository.create_conversation(data)
    ...
    def get_conversation(
        ...
        if record is None:
            raise HTTPException(
                status_code=404,
                detail=(f"Chat conversation '{conversation_id}' was not found."),
            )
    ...
    def update_conversation(
        ...
        record = self.repository.update_conversation(
            conversation_id,
            data,
        )
    ...
    async def delete_conversation(
        ...
        if self.attachment_service is not None:
            await self.attachment_service.cleanup_conversation_attachments(
                conversation_id
            )

        deleted = self.repository.delete_conversation(conversation_id)
    ...
    def create_message(
        ...
        except ChatMessageAttachmentBindingError as error:
            raise HTTPException(
                status_code=409,
                detail=str(error),
            ) from error
